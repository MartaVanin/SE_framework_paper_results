using JuMP, PowerModelsDistribution, PowerModels, PowerModelsDistributionStateEstimation
...
function run_linear_ivr_red_mc_se(data::Union{Dict{String,<:Any},String}, solver; kwargs...)
    return run_mc_selin(data, PowerModelsDistributionStateEstimation.ReducedIVRPowerModel, solver; kwargs...)
end
...
function build_mc_selin(pm::PowerModelsDistributionStateEstimation.ReducedIVRPowerModel)
    # Variables
     ...
     PowerModelsDistributionStateEstimation.variable_mc_gen_power_setpoint_se(pm, bounded = true)
    ...
    for id in PowerModelsDistribution.ids(pm, :gen)
        PowerModelsDistributionStateEstimation.constraint_mc_gen_setpoint_se(pm, id; bounded = false)
    end

    for (i,bus) in PowerModelsDistribution.ref(pm, :bus)
        PowerModelsDistributionStateEstimation.constraint_mc_load_current_balance_se(pm, i)
    end
